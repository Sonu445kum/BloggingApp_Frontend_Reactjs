...
const BlogList = () => {
  ...
  // ✅ Fetch blogs with pagination (from backend)
  const {
    data: blogsData,
    isLoading,
    isError,
    refetch,
  } = useGetBlogsQuery(currentPage);
  ...
  // 🟩 UPDATED: Normalize Data based on DRF pagination structure
  const blogs = blogsData?.results || []; // <-- Prevents .map() error
  const totalCount = blogsData?.count || 0; // DRF returns 'count'
  const totalPages = Math.ceil(totalCount / 10); // assuming PAGE_SIZE=10
  ...
  return (
    <div className="max-w-7xl mx-auto mt-10 px-4">
      ...
      {/* 🔹 Pagination Controls */}
      {/* 🟩 UPDATED: Using count-based totalPages */}
      <Paginations
